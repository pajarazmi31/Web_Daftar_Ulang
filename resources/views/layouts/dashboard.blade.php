        ...
        <aside
            ...
            <div>
                ...
                <nav class="p-4 space-y-1">
                    ...
                    {{-- ADMIN & OPERATOR --}}
                    @if(in_array(auth()->user()->role->nama_role, ['admin', 'operator']))
                    ...
                    <a href="{{ route('data-peserta') }}"
                        class="flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition group {{ request()->routeIs('data-peserta') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/10' : 'hover:bg-slate-800/50 hover:text-white text-slate-400' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" />
                        </svg>
                    ...
                    <a href="#" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800/50 hover:text-white text-slate-400 font-medium transition group">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="w-full">
                        @csrf
                        <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition group text-slate-400 hover:bg-slate-800/50 hover:text-rose-400">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        ...
                        <div
                            ...
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button class="w-full flex items-center gap-2.5 px-4 py-3 text-sm font-medium text-rose-600 hover:bg-rose-50/60 transition text-left">
