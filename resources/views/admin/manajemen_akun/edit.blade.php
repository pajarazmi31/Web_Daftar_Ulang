...
@section('content')
<div class="max-w-3xl bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
    ...
    <form action="{{ route('manajemen_akun.update', $user->id) }}" method="POST" class="p-6 space-y-5 text-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="w-full px-4 py-2.5 border @error('name') border-rose-400 focus:border-rose-500 @else border-slate-100 focus:border-indigo-500 @enderror rounded-xl focus:outline-none transition">
                @error('name') <p class="text-xs text-rose-500 mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-slate-600 font-medium mb-1.5">
                    {{ $user->role_id == 3 ? 'NISN / Username Siswa' : 'Alamat Email Resmi' }}
                </label>
                <input type="{{ $user->role_id == 3 ? 'text' : 'email' }}"
                    name="email"
                    value="{{ old('email', $user->email) }}"
                    required
                    placeholder="{{ $user->role_id == 3 ? 'Masukkan NISN Siswa' : 'contoh@sekolah.sch.id' }}"
                    class="w-full px-4 py-2.5 border @error('email') border-rose-400 focus:border-rose-500 @else border-slate-200 focus:border-indigo-500 @enderror rounded-xl focus:outline-none transition">
