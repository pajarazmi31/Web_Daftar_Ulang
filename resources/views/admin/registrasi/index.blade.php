...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 mb-6">
    ...
    <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
        ...
        <div>
            <span class="block text-xs font-medium text-slate-400 uppercase tracking-wider">Belum Registrasi</span>
            <span class="text-2xl font-bold text-slate-700">{{ $countBelumRegistrasi ?? 0 }}</span>
        </div>
    </div>
</div>
...
<form action="{{ url()->current() }}" method="GET" class="w-full bg-white p-4 rounded-2xl border border-slate-100 mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between shadow-sm">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-3 w-full lg:max-w-4xl">
        ...
        <div class="relative flex items-center w-full">
            ...
            <input
                ...
                placeholder="Cari..."
                class="w-full pl-10 pr-16 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 focus:bg-white transition-all text-slate-700">

            <div class="absolute inset-y-0 right-0 flex items-center pr-4 gap-2">
                @if(request('search') || request('jurusan') || request('status'))
                <a href="{{ url()->current() }}" class="text-xs text-slate-400 hover:text-slate-600 font-medium transition-colors" title="Clear semua filter">
                    Reset
                </a>
                <span class="h-4 w-px bg-slate-200"></span>
                @endif
                <button type="submit" class="text-xs text-slate-600 hover:text-slate-800 font-semibold transition-colors">Cari</button>
            </div>
        </div>
        ...
        <div class="w-full">
            <select
                ...
                class="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 focus:bg-white text-slate-700">
        ...
        <div class="w-full">
            <select
                ...
                <option value="Belum Registrasi" {{ request('status') == 'Belum Registrasi' ? 'selected' : '' }}>Belum Registrasi</option>
        ...
        <div class="w-full lg:w-auto">
            <a href="{{ route('registrasi.create') }}" class="w-full lg:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-medium shadow-sm transition-all duration-150 whitespace-nowrap">
                ...
                <tr class="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center justify-center gap-1">
                            ...
                            @if(($item->status_registrasi ?? '') !== 'Belum Registrasi')
                            ...
                            @else
                            <span class="text-xs text-slate-400 font-medium bg-slate-100 px-2 py-1 rounded-md">Belum Ada Aksi</span>
                            @endif
